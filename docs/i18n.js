...
const I18N = {
    es: {
        ...
        'What the installers at the top of this page give you: a Storage SCP that receives studies, and a folder watcher that forwards them. Nothing else.': 'Lo que le dan los instaladores del principio de esta página: un Storage SCP que recibe estudios y un vigilante de carpeta que los reenvía. Nada más.',
        'Next — and running today': 'La siguiente, y ya funciona hoy',
        'Full changelog': 'Registro de cambios completo',
        'Not scheduled': 'Sin fecha',
        'Encryption at rest, and directory integration (LDAP, Active Directory, single sign-on). Both are deliberate gaps rather than things forgotten — the security policy sets out the reasoning, and an encrypted disk underneath covers the first one today.': 'Cifrado en reposo e integración con su directorio (LDAP, Active Directory, inicio de sesión único). Ambas son ausencias deliberadas, no olvidos: la política de seguridad explica el razonamiento, y un disco cifrado por debajo cubre hoy la primera.',
    },
    'pt-BR': {
        ...
        'What the installers at the top of this page give you: a Storage SCP that receives studies, and a folder watcher that forwards them. Nothing else.': 'O que os instaladores do topo desta página oferecem: um Storage SCP que recebe estudos e um vigia de pasta que os encaminha. Nada mais.',
        'Next — and running today': 'A próxima — e já funciona hoje',
        'Full changelog': 'Registro de mudanças completo',
        'Not scheduled': 'Sem data',
        'Encryption at rest, and directory integration (LDAP, Active Directory, single sign-on). Both are deliberate gaps rather than things forgotten — the security policy sets out the reasoning, and an encrypted disk underneath covers the first one today.': 'Criptografia em repouso e integração com o seu diretório (LDAP, Active Directory, logon único). As duas são ausências deliberadas, não esquecimentos — a política de segurança explica o raciocínio, e um disco criptografado por baixo já cobre a primeira.',
    },
    ja: {
        'Late shift.': '夜勤お疲れさま。',
        ...
        'Releases': 'リリース',
        'Available now': '現在入手可能',
        'What the installers at the top of this page give you: a Storage SCP that receives studies, and a folder watcher that forwards them. Nothing else.': 'このページ上部のインストーラーで手に入るもの。検査を受信する Storage SCP と、それを転送するフォルダ監視だけです。',
        'Next — and running today': '次のリリース — 今日すでに動きます',
        'Full changelog': '変更履歴の全文',
        'Not scheduled': '予定なし',
        'Encryption at rest, and directory integration (LDAP, Active Directory, single sign-on). Both are deliberate gaps rather than things forgotten — the security policy sets out the reasoning, and an encrypted disk underneath covers the first one today.': '保存時暗号化と、ID 基盤との連携（LDAP・Active Directory・シングルサインオン）。どちらも忘れているのではなく意図的に見送っている項目です。理由はセキュリティポリシーに記載しており、前者は下層で暗号化したディスクを使えば今日でも対応できます。',
    },
    ru: {
        ...
        'What the installers at the top of this page give you: a Storage SCP that receives studies, and a folder watcher that forwards them. Nothing else.': 'То, что дают установщики в начале этой страницы: Storage SCP, который принимает исследования, и наблюдатель за папкой, который их пересылает. Больше ничего.',
        'Next — and running today': 'Следующий — и уже работает',
        'Full changelog': 'Полный список изменений',
        'Not scheduled': 'Не запланировано',
        'Encryption at rest, and directory integration (LDAP, Active Directory, single sign-on). Both are deliberate gaps rather than things forgotten — the security policy sets out the reasoning, and an encrypted disk underneath covers the first one today.': 'Шифрование на диске и интеграция с каталогом (LDAP, Active Directory, единый вход). И то и другое — осознанные пробелы, а не забытое: политика безопасности объясняет почему, а первый уже сегодня закрывается зашифрованным диском под системой.',
    },
};
...
const RICH = {
    '#rel-next': {
        es: 'Todo lo demás de esta página. Está escrito, probado y fusionado en <code>main</code>; lo que falta es una versión etiquetada con instaladores firmados. Ejecútelo ya con Docker o desde el código.',
        'pt-BR': 'Todo o resto desta página. Está escrito, testado e integrado em <code>main</code>; o que falta é uma versão marcada com instaladores assinados. Rode agora com Docker ou a partir do código.',
        ja: 'このページに載っている残りすべて。すでに実装・テストを終えて <code>main</code> に入っており、足りないのは署名付きインストーラーを伴うタグ付きリリースだけです。Docker またはソースから今日すぐ動かせます。',
        ru: 'Всё остальное с этой страницы. Написано, протестировано и влито в <code>main</code>; не хватает только тегированного выпуска с подписанными установщиками. Запускайте уже сегодня через Docker или из исходников.',
    },
    '.lede': {
    ...
    '#dl-release': {
        es: 'Estos instaladores son <b>v1.0.0</b>: solo reciben y reenvían. Todo lo demás de esta página sale en <b>v1.1.0</b>, que ya funciona hoy con Docker o desde el código. <a href="#releases">Qué lleva cada versión ↓</a>',
        'pt-BR': 'Estes instaladores são <b>v1.0.0</b>: apenas recebem e encaminham. Todo o resto desta página sai na <b>v1.1.0</b>, que já funciona hoje com Docker ou a partir do código. <a href="#releases">O que tem em cada versão ↓</a>',
        ja: 'ここにあるインストーラーは <b>v1.0.0</b> で、受信と転送だけを行います。このページの他の機能は <b>v1.1.0</b> に入り、Docker またはソースからなら今日すでに動きます。<a href="#releases">各リリースの内容 ↓</a>',
        ru: 'Эти установщики — <b>v1.0.0</b>: только приём и пересылка. Всё остальное на этой странице выходит в <b>v1.1.0</b>, которая уже работает сегодня через Docker или из исходников. <a href="#releases">Что входит в каждый выпуск ↓</a>',
    },
};
